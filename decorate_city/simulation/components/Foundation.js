...
Foundation.prototype.InitialiseConstruction = function(template)
{
	...
	// Substitua este trecho problemático:
	// this.isDecorative = this.finalTemplateName.startsWith("template_decorate");

	// Pelo novo código:
	let cmpTemplateManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager);
	let templateData = cmpTemplateManager.GetTemplate(this.finalTemplateName);

	let rawClasses = templateData?.Identity?.Classes;
	let classString = typeof rawClasses === "object" && rawClasses._string
		? rawClasses._string
		: "";


	this.isDecorative = classString.split(" ").includes("Decorative");
...
Foundation.prototype.Commit = function()
{
	...
	let cmpObstruction = Engine.QueryInterface(this.entity, IID_Obstruction);
		if (cmpObstruction)
		{
			// Obter as entidades que estão bloqueando a construção do PAREDE/FUNDAÇÃO.
			// Note que GetEntitiesBlockingConstruction() considera BlockFoundation do alvo.
			// Precisamos, portanto, ajustar a lógica *depois* que as colisões são detectadas.
			let collisions = cmpObstruction.GetEntitiesBlockingConstruction();

			// Se a fundação *não* é decorativa, aplicamos a lógica de bloqueio padrão.
			if (!this.isDecorative)
			{
				if (collisions.length)
				{
					...
					return false; // Bloqueia a construção para fundações não decorativas
				}
				// Se não há colisões, ou se não for decorativo, permitir o movimento/pathfinding
				cmpObstruction.SetDisableBlockMovementPathfinding(false, false, -1);
			}
			else // Se a fundação *é* decorativa
			{
				for (let ent of collisions)
				{
					let cmpIdentity = Engine.QueryInterface(ent, IID_Identity);
					// Se a entidade colidida NÃO é uma Structure, então ainda bloqueamos.
					// (Ex: não permitir pavimento sobre unidades ou recursos que não sejam estruturas).
					if (!cmpIdentity || !cmpIdentity.GetClassesList().includes("Structure"))
					{
						return false;
					}
					// Se for uma Structure, a permissão para sobrepor é concedida implicitamente
					// ao não retornar 'false' aqui.
				}
				// Para decorativos, não há necessidade de chamar SetDisableBlockMovementPathfinding(false, false, -1)
				// pois seus próprios templates de Obstruction já têm BlockMovement/BlockPathfinding como false.
			}
		}
